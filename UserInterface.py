...
class MiSaSiM(Tk) :
    def __init__(Self, Version) :
        ...
        Self.Nav = Navigator(Self.CodeBase, Self.StackBase, Self.StartingSP)
        Self.Sim = Simulation(Self.CodeBase, Self.StackBase, Self.StartingSP,
                              Self.NumCores, Self.Nav)
        Self.Code = None

        # core display button functions
        Self.CurrCore = 0
        Self.CoreIDSetters = []
        for CoreID in range(Self.NumCores):
            Self.CoreIDSetters.append(
                Self.Create_CoreID_Setter(CoreID))
    ...
    def TopFileMenu(Self) :
        ...
        filemenu.add_command(label="Load", underline=0, 
                             accelerator='Ctrl-L', command=Self.CodeView.Load)
        filemenu.add_command(label="Reload", underline=0, 
                             accelerator='Ctrl-R', command=Self.CodeView.Reload)
        filemenu.add_command(label="Execute", underline=1, 
                             accelerator='Ctrl-X', command=Self.Execute)
        filemenu.add_command(label="Restart", underline=3, 
                             accelerator='Ctrl-T', command=Self.Restart)
        filemenu.add_command(label="MultiExec", underline=0, 
                             accelerator='Ctrl-M', command=Self.MultiExec)
        filemenu.add_separator()
        filemenu.add_command(label="Dump", underline=0, 
                             accelerator='Ctrl-D', command=Self.Dump)
        filemenu.add_separator()
    ...
    def TopEditMenu(Self) :
        ...
        editmenu.add_command(label="Options", underline=0, 
                             accelerator='Ctrl-O', command=Self.Options)
        Self.menubar.add_cascade(label="Edit", menu=editmenu)
        
    def TopTraceMenu(Self) :
        TraceMenu = Menu(Self.menubar, tearoff=0)
        TraceMenu.add_command(label="Trace Start", underline=6, 
                              accelerator='Ctrl-S', command=Self.Goto_Start)
        TraceMenu.add_command(label="Trace End", underline=6, 
                              accelerator='Ctrl-E', command=Self.Goto_End)
        TraceMenu.add_separator()
        TraceMenu.add_command(label="Previous", underline=0, 
                              accelerator='Ctrl-P', command=Self.Prev)
        TraceMenu.add_command(label="Next", underline=0, 
                              accelerator='Ctrl-N', command=Self.Next)
        TraceMenu.add_separator()
        TraceMenu.add_command(label="Backward", underline=0, 
                              accelerator='Ctrl-B', command=Self.Backward)
        TraceMenu.add_command(label="Forward", underline=0, 
                              accelerator='Ctrl-F', command=Self.Forward)
    ...
    def MultiExec(Self, Event=None) :
        """ This routine queries for a repeat value. It then execute the
        program multiple times (as specified) and returns the summary
        statistics. """
        raise NotImplementedError("Not supported for multi-core yet")
        MultiExecWin = Toplevel()
        MultiExecWin.title('MultiExecution')
        RepRateScale = Scale(MultiExecWin, 
                             label='Enter number of executions', 
                             from_=1, to=100, orient='horizontal', length=400)
        RepRateScale.pack(expand=YES, fill=X)
        ButtonFrame = Frame(MultiExecWin)
        ButtonFrame.pack(side=TOP, fill=X)
        OKButton= Button(ButtonFrame, text='OK', 
                         command=lambda: Self.Set_Rep_Rate(
                             RepRateScale, MultiExecWin))
        OKButton.pack(side=RIGHT, expand=YES, fill=X)
        CancelButton = Button(ButtonFrame, text='Cancel', 
                              command=MultiExecWin.destroy)
        CancelButton.pack(side=RIGHT, expand=YES, fill=X)

    def Set_Rep_Rate(Self, Scale, Win) :
        """ This callback routine sets the rep rate for executions. The
        executions are then performed and the reprate is reset to one."""
    ...
    def Execute(Self, Event=None) :
        ...
        if Self.Sim.CodeLoaded:
            if not Self.Nav.HasTrace():
                Self.Restart()
            Self.Sim.Simulate(Self.ExeLimit)
            Self.Nav.Goto_Start_of_Trace()
            Self.TraceView.Refresh()
            Self.RegView.Refresh()
            DataImage, StackImage = Self.DataView.Get_Init_Mem_Image()
            Self.DataView.Refresh(DataImage)
            Self.StackView.Refresh(StackImage)
            #SumDynamicI = 0.0
            #SumStackData = 0.0
            #SumDynamicMix = None
    ...
    def Clear(Self, Event=None) :
        ...
        views. """
        Self.Sim = Simulation(
            Self.CodeBase, Self.StackBase, Self.StartingSP,
            Self.NumCores, Self.Nav)
        Self.MenuView.Refresh()
        Self.CodeView.Refresh()
        Self.TraceView.Refresh()
    ...
    def Options(Self, Event=None) :
        ...
        for Size in (8, 10, 11, 12, 14, 16, 18) :
            FontSizeEntry = Radiobutton(FontSizeFrame, text=str(Size), 
                                        variable=FontSize, value=Size)
        ...
        for Face in ('normal', 'bold', 'italic') :
            FontFaceEntry = Radiobutton(FontFaceFrame, text=Face, 
                                        variable=FontFace, value=Face)
        ...
        for Mode in ('icon', 'name', 'icon+name') :
            ButtonModeEntry = Radiobutton(ButtonModeFrame, text=Mode, 
                                          variable=ButtonMode, value=Mode)
        ...
        for Text, Base in (("decimal", 10), ("hexidecimal", 16)) :
            DisplayBaseEntry = Radiobutton(DisplayBaseFrame, text=Text, 
                                           variable=DisplayBase, value=Base)
        ...
        ButtonFrame.pack(side=TOP, fill=X)
        OKButton = Button(ButtonFrame, text='OK', 
                          command=(lambda Vars=OptionsVars, Win=OptionsForm:
                                   Self.Update_Options(Vars, Win)))
        OKButton.pack(side=LEFT, expand=YES, fill=X)
        CancelButton = Button(ButtonFrame, text='Cancel', 
                              command=OptionsForm.destroy)
        CancelButton.pack(side=RIGHT, expand=YES, fill=X)
    ...
    def Update_Display_Fonts(Self) :
        ...
        Self.MenuView.IP.config(
            font=('courier', Self.FontSize, Self.FontFace))
        Self.CodeView.Code.config(
            font=('courier', Self.FontSize, Self.FontFace))
        Self.TraceView.Trace.config(
            font=('courier', Self.FontSize, Self.FontFace))
        for Name in range(32) :
            Self.RegView.Labels[Name].config(
                font=('courier', Self.FontSize, Self.FontFace))
        Self.DataView.Mem.config(
            font=('courier', Self.FontSize, Self.FontFace))
        Self.StackView.Mem.config(
            font=('courier', Self.FontSize, Self.FontFace))
        Self.MessageView.Msgs.config(
            font=('courier', Self.FontSize, Self.FontFace))

    def Load_Init_File(Self) :
        """ This routine loads the current options settings to the options.ini
        file (if it exists). """
        Path = os.getcwd()
    ...
    def Store_Init_File(Self) :
        """ This routine saves the current options settings to the options.ini
    ...
    def Dump(Self, Event=None) :
        ...
        FileName = asksaveasfilename(initialdir='../asm', 
        ...
        if FileName:
            try :
                ...
                Self.DataView.Dump_Memory(File)
                Self.StackView.Dump_Memory(File)
                File.close()
    ...
    def Next(Self, Event=None) :
        ...
        if Self.Sim.CodeLoaded:
            if not Self.Nav.HasTrace():
                Self.Execute()
            Mods = Self.Nav.Move_to_Next_Occurrence()
            Self.Process_Mods(Mods, True)
            del Mods
    ...
    def Prev(Self, Event=None) :
        ...
        if Self.Sim.CodeLoaded:
            if not Self.Nav.HasTrace():
                Self.Execute()
            Mods = Self.Nav.Move_to_Prev_Occurrence()
            Self.Process_Mods(Mods, False)
            del Mods
    ...
    def Goto_Addr(Self, Event=None) :
        ...
        ButtonFrame.pack(side=TOP, fill=X)
        OKButton = Button(ButtonFrame, text='OK', 
                          command=(lambda Vars=GotoVars, Win=GotoForm:
                                   Self.Goto_Addr_Cmd(Vars, Win)))
        OKButton.pack(side=LEFT, expand=YES, fill=X)
        CancelButton = Button(ButtonFrame, text='Cancel', 
                              command=GotoForm.destroy)
        CancelButton.pack(side=RIGHT, expand=YES, fill=X)
    ...
    def Goto_Addr_Cmd(Self, Vars, Window) :
        ...
        if Inst_Addr > 0 and Inst_Addr % 4 == 0 :
            if not Self.Sim.Instructions == [] :
                if Self.Sim.Trace == [] :
                    Self.Execute()
                Mods = Self.Nav.Move_to_Address(Inst_Addr)
                Self.Process_Mods(Mods, True)
                del Mods
        Window.destroy()
    ...
    def Process_Mods(Self, Mods, Forward) :
        ...
        This routine processes a Mod oject returned from the Navigator.  The
        direction flag Forward indicates the direction of the trace motion. 
...
Load = \
"""R0lGODlhEAAQAOYAAP///v/9+v/9+P7+/f789/788v789v368v777/767/368fv26Pv36fv36vz2
...
Reload = \
"""R0lGODlhEAAQAOYAAKfXnqTUm6HSmJ/Sl5/Rlp7RlZzQlJzPlJrPkpnPkJjOj5bNj5XMjZTLjJLK
...
Exec = \
"""R0lGODlhEAAQAOcAAP///ykpKUyJxkqHxBxLk2RkZEiEwkWAvygxPZ6/5qC/5T4+PikqLCksMCky
...
MultiExec = \
"""R0lGODlhEAAQAOcAAP///zdrGSkpKUyJxkqHxKC/5Z6/5rLSlY26ZI+8Z0iEwkWAv1OMKDBlqTFE
...
Dump = \
"""R0lGODlhEAAQAPebADZrvDZruzZquzlsvDtuvTptuzhruz5wuzVptv///9Hg9tHg9/j7/vf7/vb5
...
Start = \
"""R0lGODlhEAAQAOYAALTO8rDL8KvI76XE7aLB7p7A652+7Ji76Za56pG26JC054+155C15o+06I6z
...
Prev = \
"""R0lGODlhEAAQANUAAJq96pm76ZW46JW56Iuz5Y2y5oav5YOt44Gr43yp4Xqm5Xqm4Xek4HWk43Gh
...
Forward = \
"""R0lGODlhEAAQANUAAK3I76jH7qPC7aHB7Jm76pm76ZK36JC154qx5Iew6Iev5IGr44Gr4oCr6oCr
...
End = \
"""R0lGODlhEAAQAOYAAK3I76vI76XE7abD7p7A656+6pq86pi76Ze66pS36JK36JG26Iuz54uy5Y2y
...
Goto = \
"""R0lGODlhEAAQAOcAAP///3un3JK56Pz+/vv9/VFxqff5+/X5+/T3+/n7/KrLkfr9/fz9/f3//
...
Options = \
"""R0lGODlhEAAQAOcAAP///5ubm6O96MfHx+Tk5Ji05lGD1ZaWlpOTk5KSkoeHh4WFhYGk33Sc3nie
...
Exit = \
"""R0lGODlhEAAQAOcAAP////90d/heYfhfYvddYPtpbOIdH/dbXf93evZYW/VWWfNRVOovMfBHSu5B
...
class MenuPane(Frame) :
    ...
    def BuildMenu(Self, Parent) :
        Self.IconImages = []
        for Image, Text, Command in (
            (Load, ' Load ', lambda: Parent.CodeView.Load()),
            ...
            if Self.master.ButtonMode == 'name' :
                Button(Self, bd = 2, text = Text, 
                       compound = LEFT, relief = GROOVE, 
                       overrelief= RAISED, command = Command).pack(side = LEFT)
            elif Self.master.ButtonMode == 'icon+name' or \
                    Self.master.ButtonMode == 'icon':
                IconImage = PhotoImage(data = Image)
                # keep images to avoid garbage collection
                Self.IconImages.append(IconImage) 
                if Self.master.ButtonMode == 'icon+name' :
                     Button(Self, bd = 2, text = Text, 
                            image = IconImage, compound = LEFT, 
                            relief = GROOVE, overrelief= RAISED, 
                            command = Command).pack(side = LEFT)
                else :
                     Button(Self, bd = 2, image = IconImage, 
    ...
    def BuildCoreInfo(Self, Parent) :
        ...
        Self.CurrCore = Label(Self, textvariable = Self.CoreText, 
                              height=1, width=10)
...
class CodePane(Frame) :
    def __init__(Self, Parent) :
        ...
        for Tag, Color in (
            ('Address', 'black'), ('Label', 'blue'), ('Opcode', 'purple'),
    ...
    def Refresh(Self) :
        ...
        if Self.master.Code != None:
            for I in Self.master.Code.Instructions :
    ...
    def Select(Self, Row) :
        ...
        This routine removes the current 'Select' tagged region. It then
        selects and highlight the indicated row. 
    ...
    def Load(Self, Event=None) :
        ...
        if not Self.CurrentDialog :
            Self.CurrentDialog = Open(
                initialdir='../asm', filetypes=[('assembly files', '.asm')])
        ...
        if FileName:
            try :
                ...
                Parser = InstParser()
                Self.master.Code = Parser.Parse_Program(
                    File, Self.master.CodeBase)
                Self.master.Sim.Load_Code(Self.master.Code)
...
class TracePane(Frame) :
    ...
    def Refresh(Self) :
        ...
        Self.Print_Trace_Block()                            
        # start at first trace entry
        Self.Select(0)                                      
        Self.Trace.config(state='disabled')
    ...
    def Select(Self, Row) :
        ...
        This routine removes the current 'Select' tagged region. Then the
        needed block is printed (unless already available). It the slected row
        is highlighted. 

        """
        Self.Trace.config(state='normal')
        # clear select mark
        Self.Trace.tag_remove('Select', '1.0', END)         
        if not Self.Base <= Row < Self.Base + Self.Length : 
            # if row not in current block
            # then compute new base
            # and print new block
            Self.Base = (Row / Self.Length) * Self.Length   
            Self.Print_Trace_Block()                        
        # compute block offset
        Offset = Row % Self.Length                          
    ...
    def Print_Trace_Block(Self) :
        ...
        if Self.master.Nav.Trace <> [] :
            LastAddress = 0
            AddressTag = 'Even'     # address backgroung color toggle value
            Block = Self.master.Nav.Trace[Self.Base:Self.Base + Self.Length]
            for CoreID, InstIP, Where, Old, New in Block :
                I = Self.master.Code.Lookup_Instruction(InstIP)
                for (Tag, String) in I.Tagged_Print() :
                    if not Tag == 'Comment' :               
                        # don't print comments
                        # save insert point
                        Start = Self.Trace.index(INSERT)    
                        # print I substring
                        Self.Trace.insert(INSERT, String)   
                        if Tag :
                            # tag substring
                            Self.Trace.tag_add(Tag, Start, INSERT)  
                            if Tag == 'Address' :
                                Address = int(String)
                                if not (Address - LastAddress) == 4 :   
                                    # if non-sequential
                                    # toggle tag color
                                    if AddressTag == 'Even' :           
                                        AddressTag = 'Odd'
                ...
                if New <> None and Old <> None and I.Opcode <> 'swi':
                    Self.Trace.insert(
...
class RegPane(Frame) :
    def __init__(Self, Parent) :
        ...
        Self.Labels = []
        Self.Values = []
        #Register table maintains register status of all cores.
        Self.RegTable = {}
        for CoreID in range(Self.master.NumCores):
...
class MemoryPane(Frame) :
    ...
    def Refresh(Self, InitMemoryImage) :
    ...
    def Dump_Memory(Self, Fd):
        for Address in sorted(MemImage.keys()):
            Value = Self.MemImage[Address]
            Fd.write('%04d: %10d\n' % (Address, Value))
    ...
    def Process_Mods(Self, Mods, Forward) :
        ...
        #apply change to the image first
        for Address, State in Mods.iteritems():
            Value, Allocated = State
            if Allocated:
                if not Forward:
                    del Self.MemImage[Address]
                    continue
            Self.MemImage[Address] = Value
        #update the view
        SortedAddresses = sorted(Self.MemImage.keys())
    ...
    def Gray_Free_Memory(Self, SortedAddresses) :
        ...
        unabllocated rows. """
        #print ('stack address maintainance broken here for mt, '
        #       'Gray_Free_Memory malfunctioning')
        return
        Sim = Self.master.Sim
    ...
    def Paint_Active_Frame(Self, SortedAddresses) :
        ...
        #print ('active frame address maintainance broken here for mt, '
        #       'Gray_Free_Memory malfunctioning')
        return
        Sim = Self.master.Sim
    ...
    def Paint_Static_Data(Self, SortedAddresses) :
        ...
        #print ('static data address maintainance broken here for mt, '
        #       'Gray_Free_Memory malfunctioning')
        return
        Sim = Self.master.Sim
    ...
    def Get_Init_Mem_Image(Self):
        DataImage = {}
        StackImage = {}
        for Address, State in Self.master.Sim.InitMemImage.iteritems() :
            if Address < Self.master.StackBase :
                DataImage[Address] = State
            else :
                StackImage[Address] = State
        return DataImage, StackImage
